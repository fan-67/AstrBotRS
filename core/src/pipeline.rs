...
impl Pipeline {
    ...
    pub async fn execute(&self, event: AstrMessageEvent) {
        ...
        if let Err(e) = platform.start_typing(&session_id).await {
            debug!("start_typing failed for {session_id}: {e}");
        }

        let req = astrbot_provider::ProviderRequest::prompt(&text);
        match provider.text_chat(req).await {
            Ok(response) => {
                if let Err(e) = platform.stop_typing(&session_id).await {
                    debug!("stop_typing failed for {session_id}: {e}");
                }
            ...
            Err(e) => {
                if let Err(stop_e) = platform.stop_typing(&session_id).await {
                    debug!("stop_typing failed for {session_id}: {stop_e}");
                }
